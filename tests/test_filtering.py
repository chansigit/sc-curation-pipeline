...
def _setup(tmp_path, folder_name, counts, *, min_genes_per_cell=2, min_cells=1):
    ...
    # write the upstream standardized file where h5ad_qc would have put it
...
def _materialize(src, key, settings, instance):
    return dg.materialize(
        [cell_filtered], partition_key=key, instance=instance,
        resources={"curation": settings}, tags={"sc/h5ad_path": src},
        raise_on_error=False,
    )


def test_cell_filtered_writes_and_counts(tmp_path):
    ...
    md = res.asset_materializations_for_node("cell_filtered")[0].metadata
...
def test_cell_filtered_too_few_after_fast_fail(tmp_path):
    ...
    msg = [e for e in res.get_step_failure_events()
           if e.step_key == "cell_filtered"][0].event_specific_data.error.message
    assert "min_cells" in msg
    assert not os.path.isfile(filtered_path_for(std))  # no output written


def test_cell_filtered_missing_standardized_fails(tmp_path):
    watch = str(tmp_path / "watch"); out = str(tmp_path / "out")
