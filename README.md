...
- **元数据列识别(stanmetacols)**:`metacols_method`(走了 LLM 还是启发式)、`metacols_sample` / `metacols_cell_type_coarse` / `metacols_cell_type_fine`(各角色被规范化时选中的源列名,`—` 表示没识别到)。详见下方「元数据列识别」。
...
- `obs` 新增每细胞 `pct_counts_mt` / `pct_counts_hb`(物种感知污染比例),以及识别到的规范元数据列 `sample` / `cell_type_coarse` / `cell_type_fine`(见下「元数据列识别」);`obsm` / `obsp` 不变(`var` 另含基因名标准化的映射列,见下)
...
### 元数据列识别(`h5ad_qc` step 内,stanmetacols)

`h5ad_qc` 用 [stanmetacols](https://github.com/chansigit/stanmetacols) 识别 `.obs` 里填充标准**元数据角色**的列,只针对 pipeline 自己不算的三个角色:`sample`、`cell_type_coarse`、`cell_type_fine`(QC 数值角色 pct_mt/pct_hb 等由本步自算,不交给它)。

- **识别 + 规范化**:对每个角色取 top-1 候选,**仅当它是单列(`single`)、在 obs 里、且分数 ≥ 0.5** 时,把该列复制成同名规范列 `obs["sample"]` / `obs["cell_type_coarse"]` / `obs["cell_type_fine"]`(**原列保留**)。`composite`(`"a + b"`)/ `barcode`(`"<barcode:…>"`)候选和低分候选**只记录、不自动建列**。
- **完整排名**写进 `uns["metacols"]`(JSON 字符串),含 `method` / `assigned`(角色→源列)/ `ranking`(每角色候选列表);Dagster metadata 也给出 `metacols_method` 和各角色选中的源列名。
- **LLM vs 启发式**:`SC_CURATION_METACOLS_USE_LLM`(默认 `1` = LLM 优先)。LLM 路径走 stanmetacols 默认后端(Anthropic / `claude-opus-4-8`),读 `ANTHROPIC_API_KEY`;没 key / 无外网时自动回退离线启发式,`metacols_method` 会如实写明。计算节点跑批想要确定可复现、零外部调用,就设 `SC_CURATION_METACOLS_USE_LLM=0`(测试即用此)。
