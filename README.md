...
- **LLM vs 启发式**:`SC_CURATION_METACOLS_USE_LLM`(默认 `1` = LLM 优先)。LLM 路径走 stanmetacols 默认后端(Anthropic / `claude-opus-4-8`),读 `ANTHROPIC_API_KEY`;没 key / 无外网时自动回退离线启发式,`metacols_method` 会如实写明。
