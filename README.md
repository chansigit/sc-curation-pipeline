...
- **选 HVG**:在 raw counts 上 `sc.pp.highly_variable_genes(flavor="seurat_v3", n_top_genes=SC_CURATION_MRVI_N_HVG, batch_key="sample")` 选 top-N 高变基因(默认 2000,按 sample 分批选)。**只用 HVG 子集喂 MrVI 训练**,用 `inplace=False` 不改原对象;`n_hvg=0` 或基因数不足时用全基因。
- **MrVI**:`MRVI.setup_anndata(layer="counts", sample_key="sample", backend="torch")`(喂 HVG 子集副本)→ 训练 → `get_latent_representation(give_z=False)` 取 **u latent** → 贴回全基因对象的 `obsm["X_mrvi_u"]`。没 `sample` 列时建个常量列退化成单样本。
- **Leiden**:`sc.pp.neighbors(use_rep="X_mrvi_u")` + `sc.tl.leiden(flavor="igraph")` → `obs["mrvi_leiden"]`。
- **写**:`obsm["X_mrvi_u"]` + `obs["mrvi_leiden"]` **就地写回** `*_filtered.h5ad`(**保留全部基因**,只多这两项;全部细胞,doublet 不剔)。
- **失败**:Slurm 作业非 `COMPLETED` → `dagster.Failure`(可重试)。
- **sbatch 资源(env 可配,`-G 1` 固定)**:

  | env | 默认 | 说明 |
  |---|---|---|
  | `SC_CURATION_MRVI_PARTITION` | `gpu` | sbatch `-p`;**必须是支持 batch 的 GPU 分区**(`gpu`)。⚠️ `dev` 只收交互式(salloc/srun),sbatch 会被直接拒 |
  | `SC_CURATION_MRVI_TIME` | `01:00:00` | `--time`(短→更易被 backfill 提前调度;大数据加长) |
  ...
  | `SC_CURATION_MRVI_N_HVG` | `2000` | 训练用的 top-N HVG(seurat_v3);`0`=全基因 |
  | `SC_CURATION_LEIDEN_RESOLUTION` | `1.0` | Leiden 分辨率 |

- **metadata**(由外部作业经 Pipes 回传):`n_cells` / `n_samples` / `n_clusters` / `latent_dim` / `n_genes_total` / `n_genes_trained` / `n_hvg` / `leiden_resolution` / `accelerator`(gpu/cpu)/ `had_sample_column`。
